...
def getItemsForSearch(priceListName,txt):
    ...
    for itemCode in searchItems:
        ...
        # image = frappe.get_doc("File",'Home/Attachments')
...
@frappe.whitelist()
def get_items_for_search_PI(priceListName,txt):
    # Get defalut warehouse for each item and get the actual_qty from the mentioned
    # warehouse
    # allItems = frappe.get_all("Item")
    if isinstance(txt, str):
    ...
    for itemCode in searchItems:
        ...
        # if itemPrices:
        itemPrice = frappe.db.get_value("Item",
        filters={"item_code": itemCode['item_code']},
